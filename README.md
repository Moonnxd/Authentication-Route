...
**Why two tokens?** If an access token gets stolen, the damage window is small (it expires in 15 minutes). The refresh token is used far less often and can be revoked server-side at any time (e.g. on logout), so it's safer to keep it long-lived.
...
2. **Login** → credentials are checked → if valid, an access token and refresh token are generated and set as httpOnly cookies. The refresh token is also saved in the DB against that user.
...
This project expects a `users` table shaped like this:
...
The `refresh_token` column is what allows the server to revoke a refresh token on logout, rather than trusting the JWT alone.
...
Then create your `.env` file and database table as shown above, and start the server:
...
**Responses:** `201` on success, `400` if fields are missing/invalid or username is taken, `500` on server error.
...
### `POST /api/refresh`
Issues a new access token using the refresh token stored in cookies. No body needed — reads the `refreshToken` cookie automatically.
...
2. **Login** first (`POST /api/login`) — check the **Cookies** tab (near the Send button) to confirm `accessToken` and `refreshToken` were set.
3. Call **`GET /api/test-protected`** — should return your decoded user info.
4. Call **`POST /api/refresh`** — should return a new `accessToken` (compare the cookie value before/after to confirm it changed).
...
- **JWT payloads are not encrypted** — only signed. Never put sensitive data (like `password_hash`) inside a token payload; anyone can decode it at [jwt.io](https://jwt.io).
